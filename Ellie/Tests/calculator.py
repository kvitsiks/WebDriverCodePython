...
class Calculator(unittest.TestCase):
    ...
    def test_calculator(self):
        ...
        # ERROR: Caught exception [ERROR: Unsupported command [getEval | Math.round(Math.random() * 20) | ]]
        print(num1)
        driver.find_element_by_name("calcResults").clear()
        driver.find_element_by_name("calcResults").send_keys(num1)
        driver.find_element_by_name("calplus").click()
        # ERROR: Caught exception [ERROR: Unsupported command [getEval | Math.round(Math.random() * 20) | ]]
        print(num2)
        driver.find_element_by_name("calcResults").clear()
        driver.find_element_by_name("calcResults").send_keys(num2)
